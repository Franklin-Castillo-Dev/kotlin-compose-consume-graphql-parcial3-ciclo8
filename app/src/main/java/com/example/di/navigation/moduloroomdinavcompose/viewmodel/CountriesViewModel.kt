...
class CountriesViewModel @Inject constructor(
    ...
    val _data= MutableStateFlow<List<CountriesListQuery.Country>>(emptyList())
    ...
    fun getData(){
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                val result=apolloClient.query(CountriesListQuery()).toDeferred().await()
                val launch = result.data?.countries
                if (launch == null || result.hasErrors()) {
                    Log.d(TAG,"Error  on result")
                }else{
                    result.data?.countries?.let {
                        _data.value=it
                    }
                }
